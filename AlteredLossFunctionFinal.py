...
def clean(string):
    ...
    wordsToRemove = [
        'concussion', 'laceration', 'contusion', 'lac', ' co ', 'chi', 'conc','cont','closed','clsd','sdh','cont','sah','subdural',"h'tma'","iph",'ich','usison','ussive',
        '*', 'yr', '-', ';', ',', '.', '>', '<', ':', '/', '+', '"',
        '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '?', '#'
        ]
    ...
    for path in pathArray:
        ...
        for row in input_file:
            ...
            if row['Diagnosis'] == '52':
                ...
                labeledData.append((newString, age, race, sex, 0))
...
def evaluatePredictor(examples, predictor):
    ...
    for narrative, age, race, sex, y in examples:
        currVal = predictor(narrative, age, race, sex)
        xValue = 0
...
def learnPredictor(trainExamples, testExamples, featureExtractor, numIters, eta):
    ...
    for t in range(numIters):
        for currTrain in trainExamples:
            ...
            if currTrain[4] == 1:
                scale = 1.5
            logistic = scale*((1.0/(1+math.exp(-1*margin)))-currTrain[4])
    ...
    print(evaluatePredictor(trainExamples, lambda w, x, y, z : (1.0/(1+math.exp(-1*dotProduct(featureExtractor(w, x, y, z), weights))))))
    print("")
    print("Test error is:")
    print(evaluatePredictor(testExamples, lambda w, x, y, z : (1.0/(1+math.exp(-1*dotProduct(featureExtractor(w, x, y, z), weights))))))
    print("")
    return weights
